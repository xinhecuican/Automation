...
public class TreeViewContainer extends ViewGroup implements TreeViewNotifier {
    ...
    */
    ...
    public void onItemViewChange(NodeModel<?> nodeModel){
        ...
        if(holder != null) {
            removeView(holder.getView());
            TreeViewHolder<?> holder1 = adapter.onCreateViewHolder(this, (NodeModel) nodeModel);
            holder.setView(holder1.getView());
            adapter.onBindViewHolder((TreeViewHolder)holder);
            View view = holder.getView();
            view.setElevation(Z_NOR);
            this.addView(view);
            view.setTag(R.id.item_holder,holder);
            mTreeLayoutManager.calculateByLayoutAlgorithm(mTreeModel);
            requestLayout();
        }
    }
